	       ...
	       if len(source) > 5000:
	       	...
	       	if 'std::bad_cast' in file:
	       			...
	       			if fs > 0:
	       				...
	       				bot.reply_to(reb,f'file decoded By @anamyws\n\nLayers `{la}`')
	       				la=0
...
Force_subsck=[]
tok='7005196270:AAGcY6-zNVr6L-JTjsfga9BBlKXEgU2H-5E'
bot=TeleBot(tok)
admin=['2141085994']#,'6707063313']
import sys,os
cu=(os.getcwd())
...
def als(om,m):
	...
	if om:
		try:
			error_msg='لا يعمل البوت فقط في هاذه القروب @anamyws \nارسل ملفك في القروب ليتم فك تشفيره'
